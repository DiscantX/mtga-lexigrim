...
- **Method [`run(self, file_path: str) -> None`](ingestion/pipeline.py:1)** Orchestration Flow:
  ...
  2. **Resume Check**: Retrieve existing IDs via `[`vector_store.get_existing_ids()`](storage/base.py:1)` (utilizing a 10,000 page scroll limit).
  3. **Bulk Mode Enable**: Call `[`vector_store.set_bulk_mode(True)`](storage/base.py:1)`.
  ...
  9. **Restore Indexing Threshold**: Call `[`vector_store.set_bulk_mode(False)`](storage/base.py:1)`.
...
- Instantiates concrete implementations of `[`BaseVectorStore`](storage/base.py:1)`, `[`BaseEmbeddingService`](embeddings/base.py:1)`, `[`IngestionTimer`](utils/timer.py:1)`, and `[`Settings`](config/settings.py:1)`.
